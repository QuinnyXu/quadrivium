...
4. **The obligation-vs-expenditure seam decomposition — HD 3.6, RH clause-(b). TWO seams now named (second added at HD 3.2 finalization, 2026-05-29).** HD 3.6 is the decomposition object on the funding-IN ↔ expenditure-OUT join; HD 3.2 builds the parse + spine and *names* the seams, it does **not** decompose them. The seams:

   - **Seam A — FY-basis (existing).** Federal-FY commitments vs. institution-FY spending. Per CLAUDE.md §1 analytical thread this is its own decomposition object on the join. **Consumer-hazard carried to HD 3.6 (see §8.5):** the HD 3.2 join contract must explicitly flag the FY-basis difference so a downstream consumer — including a Power BI semantic model built on the parquet — cannot misread a timing gap as funding-conversion efficiency.

   - **Seam B — institutional-grain (NEW, surfaced by the HD 3.2 spine MVP).** FedSupport reports a **system-level** obligation (`U. Michigan`, `U. Washington`, `Penn State`, `U. Minnesota`, `Rutgers`, `U. Virginia` — each a system-level recipient with no campus disambiguator in the FedSupport name), whereas HERD reports **campus-level** expenditure (distinct UNITIDs per campus). The spine's match-rate receipt isolates this as **~7.6% of the FY2023 dollar anchor** (10 institutions, counted by *distinct* ambiguous UNITID — name-variant spellings that all resolve to a single UNITID are matches, not grain ambiguities) sitting in ambiguous flagships that resolve to MULTIPLE campus UNITIDs with no data-driven flagship pick (`identity_spine_match_rate.md` §4, the best-case-ceiling table). This is **not a name-normalization miss** and **not a HERD-side coverage gap** (Seam-A / IPEDS class) — it is a genuine grain mismatch between the two surveys' reporting units. **HD 3.2 makes NO forced campus pick**: collapsing a system obligation onto a chosen flagship campus would silently misattribute dollars and would survive validation if the match-rate were computed at the wrong grain (the §8.2 no-silent-merge discipline applies to grain as well as to same-name collisions). The system-vs-campus grain is **documented now, decomposed at HD 3.6** — it is a clause-(b) decomposition object (how much of a system obligation lands at which campus is a real, sized question requiring the IPEDS keyspace's system→campus hierarchy), parallel to Seam A. The HD 3.2 join contract flags it the same way it flags the FY-basis seam: a downstream consumer must not read a system-level FedSupport obligation as joinable 1:1 to a single HERD campus expenditure.
...
5. **TWO join seams → HD 3.6 (FY-basis AND institutional-grain).** *(Principal-flagged: carry to HD 3.6 as consumer-hazards. Second seam added at HD 3.2 finalization, 2026-05-29.)* Even a perfect spine join produces a funding-IN vs. expenditure-OUT series with two named misalignments, both deferred to HD 3.6 (§5.4) and both carried as join-contract flags:

   - **Seam A — FY-basis.** Obligations are federal-FY, HERD expenditures are institution-FY; HD 3.2 must NOT silently join them as same-year-comparable. **The join contract carries an explicit FY-basis flag** (obligations marked federal-FY via a `notes`/contract field) so no downstream consumer — including a Power BI semantic model on the parquet — misreads a timing gap as funding-conversion efficiency.

   - **Seam B — institutional grain.** FedSupport reports system-level obligations; HERD reports campus-level expenditures. The spine receipt sizes this at **~7.6% of the FY2023 dollar anchor** (10 institutions, ambiguous flagships resolving to multiple campus UNITIDs, `identity_spine_match_rate.md` §4). HD 3.2 makes **no forced campus pick** — a system→campus collapse would misattribute dollars and survive validation at the wrong grain. The join contract flags system-level FedSupport rows so a consumer cannot free-join a system obligation to one campus expenditure.

   The seam decompositions themselves (clause-(b)) are **HD 3.6**. The 2 a.m. failure mode is a downstream consumer free-joining FY2023-obligations to FY2023-expenditures and reading the gap as efficiency when part of it is the FY-basis seam (A) and part is a system-vs-campus grain artifact (B) — the join contract's two flags are what prevent it.
