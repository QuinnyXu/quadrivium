...
**Companion artifact:** `data/harmonized/herd_panel.parquet` (1973–2024, 52-year field-level expenditure panel, two parallel reconstructed series).
...
1. **Era-A direct field-level expenditure series, 1973–2009 (37 years).** Read directly from era A's `Expenditures by S&E field` question, `column='Total'`. The 1972 file carries no field-level question and is preserved in the deposit but excluded from field-level analyses. (Pre-1981 era-A field-list variability across sub-fingerprints is documented as not affecting the row-level data axis; see `docs/hd_2_1_open_items.md`.)
