...
- Identifier coverage by era, reported as a **coverage statement, not an all-or-nothing invariant** (corrected 2026-05-29 — see backlog item below). Era-A: `fice`/`institution_id` populated, `ncses_inst_id`/`ipeds_unitid` NULL. Era-B: `institution_id` (FICE-style) populated on 100% of rows; `ncses_inst_id` non-null on ~98.7% of rows; **`ipeds_unitid` is canonical-but-not-complete — non-null on ~94.3% of era-B rows**, with 47 of 1,125 distinct era-B institutions (4.2%) carrying NULL `ipeds_unitid` on every row (5 more mixed). The era-B identifier is the one HERD anchors on where present; it is not universally populated.

> **HERD backlog item (surfaced 2026-05-29 by the FedSupport identity spine; resolves in the IPEDS cycle).** The pre-2026-05-29 wording of this line asserted "era-B: all three populated." That assertion is **empirically false** and is corrected above. Building the cross-survey institution-identity spine for the Federal S&E Support module (`crosswalks/_shared/institution_identity.csv`, HD 3.2) surfaced that several dollar-dominant era-B HERD institutions carry a **NULL `ipeds_unitid` at the raw NSF source** — confirmed via `etl/_load.py:530` passing `ipeds_unitid` through verbatim and a direct FY2023 raw-file probe (0-of-N non-null on every row): **Johns Hopkins University** (~$2.1B era-B R&D), **Ohio State University**, **Texas A&M University, College Station**, **Vanderbilt University**, **University of Connecticut**, **Oregon State University**, **University of Cincinnati**, among the 47. These institutions DO carry a non-null `ncses_inst_id` (45 of the 47 do), so the NCSES keyspace is complete where IPEDS is not — `ipeds_unitid` is **canonical-not-complete**. This is a finding **about HERD**, independent of FedSupport: HERD's era-B `ipeds_unitid` was previously documented as universally present and is not. Sourcing the missing UNITIDs requires the IPEDS UNITID keyspace, which is the **IPEDS-cycle deliverable** (CLAUDE.md §1 roadmap, §10 identity-spine deferral) — NOT a HERD-side fix, because the UNITID is absent from every HERD artifact (no HERD crosswalk carries institution-identity; the personnel and attribute parquets inherit the same source NULL). The gap is quantified, not corrected, here; the IPEDS migration backfills it. Evidence trail: `etl/spikes/_herd_erab_uid_coverage.py`, `etl/spikes/_fedsupport_gate_checks.py`, `validation/reports/fedsupport/identity_spine_match_rate.md` §1/§5.
