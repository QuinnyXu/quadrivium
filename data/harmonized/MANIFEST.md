...
| `9b4b748b7c5bc84f020cf594de7795af2f8939f3db1f818f9b0bc6483b6a4bc6` | 10,121,853 | `gss_pd_nfr.parquet` | GSS postdoc + non-faculty-researcher face (dataset #3): long-format, FY1972–2024, native-UNITID-keyed. **Overlapping-marginal grain** — each row tagged `measure_group` (support / demographic / degree / citizenship / nfr …); sum WITHIN a group only. Non-zero grain; field names deferred. Built by `etl/build_gss_pd_nfr.py` from `crosswalks/gss/pd_nfr_column_map.csv`. §5 PASS vs Tables 1-9b/3-2/3-4/1-2a; validation `validation/reports/gss/gss_pd_nfr_validation.md`. |
| `44e3993e8889d43863d2a0399556a13452d3e843f733b5b49ce336f66e9888e2` | 23,957,158 | `gss_race.parquet` | GSS enrollment/demographic face (dataset #3): long-format graduate students by enrollment-status × `degree_level` × gender × race, FY1972–2024, native-UNITID-keyed. Race taxonomy pre-bridged (legacy `*_98` → `*_legacy`, retained). Non-zero grain. `field_coarse`/`field_fine` populated (SEH super-broad + detailed) for 2023-active `gss_code`s via `crosswalks/gss/field_code_map.csv`; historical-only codes NULL. Built by `etl/build_gss_race.py` from `crosswalks/gss/race_column_map.csv`. §5 PASS vs Tables 4-3/1-2a; validation `validation/reports/gss/gss_race_validation.md`. |
| `5f06409186ebec3e3a3b57590742fc21de9239f59ef8e311264eadc3f05100b1` | 24,005,705 | `gss_support.parquet` | GSS funding face (dataset #3 MVP): long-format full-time graduate students by support mechanism × federal agency × fed/nonfed, FY1972–2024, native-UNITID-keyed; `degree_level` (all_grad / masters / doctoral — post-2017 split). Non-zero grain (omitted cell = structural zero). `field_coarse`/`field_fine` populated (SEH super-broad + detailed) for 2023-active `gss_code`s via `crosswalks/gss/field_code_map.csv` (count-matched to Table 4-3); historical-only codes NULL. Built by `etl/build_gss_support.py` from the crosswalk `crosswalks/gss/support_column_map.csv`. Validation: `validation/reports/gss/gss_support_validation.md`. |
...
uv run python etl/build_gss_pd_nfr_column_map.py    # crosswalks/gss/pd_nfr_column_map.csv
uv run python etl/build_gss_pd_nfr.py        # gss_pd_nfr.parquet
```

`crosswalks/gss/field_code_map.csv` (which the three GSS panel builds read to fill
`field_coarse`/`field_fine`) is a **committed crosswalk** — a cold reader uses it
directly. Its provenance generator `etl/build_gss_field_code_map.py` count-matches
each `gss_code`'s 2023 total to NSF 25-317 Table 4-3; **regenerating it needs a
prior field-less `gss_race.parquet`** (a one-time bootstrap), so the committed CSV
is canonical (not on the standard build path above).
