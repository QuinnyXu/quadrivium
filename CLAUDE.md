...
**Roadmap.** The **Federal S&E Support module** (institution-level federal S&E funding to individual higher-education institutions, drawn from the Survey of Federal Science and Engineering Support to Universities, Colleges, and Nonprofit Institutions) — the funding-IN counterpart to HERD — shipped as **v2.0** (FY2020–2023 Table 12 slice); re-based to the full FY1971–2023 NCSES Build Table series at **v3.0** (see *The analytical thread* below). **Dataset #3 is ratified: NSF GSS** (Survey of Graduate Students and Postdoctorates in S&E), the human-capital / Tier-1 face of the funding→productivity thread — **GSS-first, SED deferred** (SED is person-grain and does not join the institution-identity spine natively). **Dataset #4 is the IPEDS IC snapshot** — the authoritative-identity spine backfill for the class-(a) NULL-UNITID tier — sequenced *after* GSS rather than before it. This **inverts the prior roadmap ordering** (IPEDS was sequenced ahead of GSS); the divergence from the principal's initial A-then-B lean and its evidentiary basis are recorded in `seeds/overrides.md` §12 (2026-05-31). GSS acquisition **was blocked on NSF/NCSES survey-data availability**; NSF/NCSES resumed 2026-06-02 — the **HD 4.1 GSS gate-spike** is the ready next action (dataset #3 kickoff). Other NCSES surveys follow as the contribution-process matures. Each migration applies the Reconstructive Harmonization methodology to that survey's discontinuities; the schema and validation patterns adapt to the survey's structure, the methodology does not.
...
- **Ratified dataset #3 (Tier 1) — human-capital productivity.** PhDs produced / researchers trained per research dollar (NSF GSS — GSS-first, SED deferred as person-grain); NCSES-native, shared keyspace. Selected as the next dataset; acquisition was blocked on NSF availability, NSF/NCSES resumed 2026-06-02 — the HD 4.1 gate-spike is the ready next action (see *Roadmap* above).
...
**Acquisition-format lock (HD 3.1, locked).** Source tables that arrive as `xlsx` are converted **once, at acquisition, to CSV** — not read as `xlsx` at runtime. (The v3.0 re-based Federal S&E Support source arrived **CSV-native**, so no conversion was needed there; the lock governs any future `xlsx`-arriving source.) Reading `xlsx` through DuckDB's `excel` extension would trigger an **unpinned runtime extension fetch**, which is not genuinely dependency-free in the reproducibility sense and would break the §3 deps-pinning posture. The lock: one-time `xlsx`→`csv` conversion at acquisition, with a **PDF audit sibling** retained alongside the CSV as the human-readable provenance check. This keeps the deposit runtime to the genuinely-pinned deps (`duckdb`, `pypdf`) and preserves the cold-reader contract above.
...
The cross-survey join spine (`crosswalks/_shared/`, §10) is a **Reconstructive Harmonization clause-(a) reconstruction object** in its own right — not a free join. HD 3.1's Table-12 slice shared **no native key** with HERD, so the dataset-#2 MVP spine reconstructed identity **by name**. The **v3.0 re-base superseded this**: the full NCSES Build Table export carries a **native IPEDS UnitID across all 53 years**, so the spine pivoted from name-reconstruction to a **native-UNITID-keyed join** (FY2023 dollar-join 78.4% vs the 73.1% name-spine), keeping a name layer only to **offer** (not write) HERD era-B NULL-UNITID backfill candidates — Seam-B-gated, deferred to IPEDS #4. Name-matching is retired for FedSupport↔IPEDS.
...
- **UNITID-canonical ≠ UNITID-complete on the HERD side.** HERD carries `ipeds_unitid` only in era B (2010–2024); era-A rows (1972–2009) carry `fice` only and have UNITID NULL (per the era-handling locks above). The spine's HERD side therefore has an **era-A UNITID coverage gap**. **Dataset #2 now spans FY1971–2023 and carries native UNITID throughout on the FedSupport side**; the constraint is **HERD-side** — the join is UNITID-clean only for **2010–2023 (HERD era-B)**, while **pre-2010 FedSupport years await HERD era-A identity (the IPEDS #4 deliverable)**.
...
Cross-survey infrastructure lives under **`crosswalks/_shared/`** — most importantly the institution-identity crosswalk (the cross-survey join spine), a clean **database-level artifact** belonging to the integrated database (not to any single survey), deposited under the one concept DOI. The identity spine is scoped to the **active-survey set** (currently HERD + Federal S&E Support), not a comprehensive registry; institution-identity-over-time as a full reconstruction object (mergers, renames, ID reassignment across decades) is deferred to the IPEDS cycle, when the registry must become comprehensive.

**One database, one concept DOI (locked 2026-05-30; overturns Decision B).** The whole integrated database is **one Zenodo deposit under one concept DOI (`10.5281/zenodo.20404785`)** — the citable unit is the **integrated harmonized database, not any single survey** (*"Quadrivium v1.2 (DOI: …)"* is the correct citation form). Each GitHub release mints a **version DOI** under that single concept DOI via the standard Zenodo GitHub integration; datasets are **subtrees within the one deposit**, and adding a survey is a new **version** of the database, not a new deposit. Tags may carry a dataset slug for readability (e.g. `v3.0.0-fedsupport`) but are versions of the one database. The per-version `.zenodo.json` title/description **must be updated to the integrated-database identity *before* each release tag** — the GitHub integration full-replaces published metadata from `.zenodo.json` at tag time, and the version DOI's metadata is permanent (the one irreversible step). This **overturns Decision B** (concept-DOI-per-dataset); the override and its reasoning are logged in `seeds/overrides.md`. The per-survey **subtree** structure above is unaffected — only the DOI-per-survey scheme is reversed. The canonical cross-survey join-key hierarchy is recorded in §4 (the cross-survey institution-identity spine addendum: UNITID canonical, with `ncses_inst_id` and name+state aliases), landed with the HD 3.1 / HD 3.2 dataset-#2 evidence. The HERD-crosswalk migration (flat `crosswalks/*.csv` → `crosswalks/herd/`) happens **with** the first `fedsupport` crosswalk landing, not as a standalone churn commit — tying the path-move to a moment the read paths in `etl/build_herd_panel.py` are already being touched; the harmonized parquets need no move (HERD already uses the filename-prefix namespace).
